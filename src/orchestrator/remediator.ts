...
import { Subtask, SubtaskResult, AgentTypeSchema } from '../config/schema.js';
import { createLogger, Logger } from '../utils/logger.js';

export interface RemediationDecision {
  action: 'retry' | 'decompose' | 'skip' | 'fail';
  reason: string;
  modifiedDescription?: string;  // For retry with different approach
  newSubtasks?: Array<{          // For decompose action
    description: string;
    agentType: string;
    dependencies: number[];
  }>;
...
export interface FailedSubtaskContext {
  ...
  maxAttempts: number;
  completedSubtasks: Subtask[];  // What has already succeeded
  projectPath: string;
}
...
export class Remediator {
   ...
   */
  async analyzeFailure(context: FailedSubtaskContext): Promise<RemediationDecision> {
    this.logger.info(
      { subtaskId: context.subtask.id, attempt: context.attemptNumber, error: context.result.error },
      'Analyzing failure for remediation'
    );
    ...
    try {
      ...
      this.logger.info(
        { subtaskId: context.subtask.id, action: decision.action, reason: decision.reason },
      ...
      this.logger.error(
        { subtaskId: context.subtask.id, error: String(error) },
        'Remediation analysis failed, defaulting to retry'
      );
...
Return ONLY valid JSON:
{
  ...
  private buildAnalysisPrompt(context: FailedSubtaskContext): string {
...
Decide the best remediation action.`;
  ...
  private parseDecision(text: string): RemediationDecision {
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error('No JSON found in remediation response');
    }

    const parsed = JSON.parse(jsonMatch[0]) as RemediationDecision;
