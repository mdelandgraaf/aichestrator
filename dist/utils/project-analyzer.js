...
export class ProjectAnalyzer {
     ...
     */
    getAllFiles(dir, maxDepth, currentDepth = 0) {
     ...
     */
    detectProjectType(_projectPath, files) {
        ...
        for (const file of files) {
            ...
            if (name === 'requirements.txt' || name === 'pyproject.toml')
                return 'python';
            if (name.includes('androidmanifest.xml') || name.includes('build.gradle.kts'))
                return 'android';
        }
        return 'unknown';
    }
     ...
     */
    hasPackageManager(projectPath, detector) {
     ...
     */
    createGreenfieldResult(type, missingSetup) {
        return {
            isGreenfield: true,
            projectType: type,
     ...
     */
    detectTypeFromDescription(description) {
